...
class SettingsView(QWidget):
    def __init__(self, data_dir: Path | None = None) -> None:
        ...
        self._disk_timer.timeout.connect(self._verificar_espacio_disco)
    ...
    def _configurar_ui(self) -> None:
        ...
        self._notif_check.toggled.connect(self._al_alternar_notificaciones)
        ...
        self._sound_check.toggled.connect(self._al_alternar_sonido)
        ...
        self._disk_spin.valueChanged.connect(self._al_cambiar_umbral)
    ...
    def _al_cambiar_umbral(self, value: int) -> None:
        self._disk_threshold = value * (1024**2)

    def _al_alternar_notificaciones(self, enabled: bool) -> None:
        self._notifications_enabled = enabled
        if self._notification_service is not None:
            self._notification_service.configurar(enabled)

    def _al_alternar_sonido(self, enabled: bool) -> None:
        self._sound_enabled = enabled

    def _verificar_espacio_disco(self) -> None:
        try:
            _total, _used, free = shutil.disk_usage(Path.home())
            free_gb = free / (1024**3)
            self._disk_card.actualizar_valor(f"{free_gb:.1f} GB")
