...
from zorma.ui.dashboard.inicio_viewmodel import InicioViewModel
...
@pytest.fixture
def vm(data_dir: Path, repo: MagicMock) -> InicioViewModel:
    return InicioViewModel(data_dir, repo)


class TestDashboardViewModel:
    def test_init_creates_no_signals(self, vm: InicioViewModel):
        assert vm.obtener_ruta_vigilancia() is None
        assert not vm.esta_clasificando

    def test_set_watch_path_emits_signal(self, vm: InicioViewModel):
        p = Path("/tmp/test")
        emitted = []
    ...
    def test_watch_path_persisted(self, data_dir: Path, repo: MagicMock):
        ...
        vm = InicioViewModel(data_dir, repo)
    ...
    def test_auto_classify_default_false(self, vm: InicioViewModel):
        assert not vm.obtener_auto_clasificar()

    def test_counters_start_at_zero(self, vm: InicioViewModel):
        emitted = []
        vm.counters_changed.connect(lambda c, e: emitted.append((c, e)))
        assert len(emitted) == 0

    def test_undo_redo_state_no_manager(self, vm: InicioViewModel):
