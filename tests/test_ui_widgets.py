...
from zorma.ui.shared.widgets import (
    BotonBarraLateral,
    EstadoVacio,
    FilaLineaTiempo,
    ListaLineaTiempo,
...
class TestSidebarButton:
    ...
    def test_accepts_icon_path(self, qtbot):
        btn = BotonBarraLateral("Test", icon_path=Path("/nonexistent/icon.svg"))
        qtbot.addWidget(btn)
...
class TestCard:
    def test_renders_title_and_value(self, qtbot):
        card = Tarjeta("Clasificados", "42", COLORS["primary"])
        qtbot.addWidget(card)
        assert card.accessibleName() == "Tarjeta: Clasificados"

    def test_update_value(self, qtbot):
        card = Tarjeta("Errores", "0", COLORS["error"])
        qtbot.addWidget(card)
...
class TestEmptyState:
    def test_renders_with_button(self, qtbot):
        es = EstadoVacio(icon="📝", title="Sin datos", description="No hay datos aún", button_text="+ Crear")
        qtbot.addWidget(es)
        es.show()
        assert es.isVisible()

    def test_renders_without_button(self, qtbot):
        es = EstadoVacio(icon="📂", title="Vacío", description="No hay contenido")
        qtbot.addWidget(es)
        es.show()
        assert es.isVisible()

    def test_button_callback_invoked(self, qtbot):
        es = EstadoVacio(icon="📝", title="Test", description="Descripción", button_text="Click")
        qtbot.addWidget(es)
        callback = MagicMock()
...
class TestTimelineRow:
    ...
    def test_undo_button_when_allowed(self, qtbot, sample_result):
        row = FilaLineaTiempo(sample_result, can_undo=True)
        qtbot.addWidget(row)
        buttons = row.findChildren(QPushButton)
...
class TestTimelineFeed:
    ...
    def test_add_result_shows_row(self, qtbot):
        ...
        rows = feed.findChildren(FilaLineaTiempo)
        assert len(rows) == 1
